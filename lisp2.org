...
replace certain functions at runtime by a different implementation,
which then actually should be respected not only globally, but also by
macro expansions.
...
expansions---and consequently, laying the responsibility of not doing
...
introducing different syntax would be expanded. By this it gets clear
there is no normal form and, consequently, no reliable code walking
without macro expansion. But with macros expanded, the function calls
that should not be addressing local function definitions are already
in place. And then, after macro expansion, there is no distinction
between function calls resulting from macro expansion and function
calls subject to local function definitions. Nevertheless, the
following experiment can illustrate what to do manually to avoid name
clashes when using local functions.
...
unique names also for local functions. So it seems to be a good idea
to use the naming conventions used for global functions in emacs lisp,
prepending function names with a package name, in a similar way also
...
~with-macro-namespace~, is similar, naming a list of variables. These
variable names can then be used safely and "un-unquoted" within the
macro definition.

This is similar to the idea of /metatronic macros/[fn:3]. But there, the
variables used within macro expansion are enclosed within special syntax.
...
same time to access two variables with the same name, the first one from the
environment and the other one being declared within the macro expansion. If this was
...
it to be hidden by local function definitions. A solution to this could use an
...
their lexical environment. So here the macro expansions must make sure
to not interfere with them. This can be addressed using local variable
namespaces.
